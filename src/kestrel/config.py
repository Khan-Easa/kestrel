...
class Settings(BaseSettings):
    ...
    execute_timeout_seconds: float = Field(default=5.0, gt=0, description= "Default and hard-ceiling wall-clock timeout for each /execute call, in seconds. Applied when a request omits timeout_seconds; a supplied per-request timeout_seconds is clamped down to this value.")
    ...
    exec_spool_dir: str = Field(default="", description="Phase 8 substep 1: base directory for the stateless/execute code tempfile. Empty = system temp (default; unchanged for host-run dev + tests). Set to a host directory bind-mounted at the same absolute path into a containerized API so docker-out-of-docker --volume mounts resolve against the host daemon. Decision 8-api-image.")
